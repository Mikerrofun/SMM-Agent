...
| `/run_pipeline` | Запустить полный пайплайн генерации |
| `/last_run` | Отчёт по последнему запуску пайплайна из БД (рассылка всем подписчикам) |
| `/transcript_post` | Создать посты из транскрипции встречи (PDF) |
| `/status` | Статус системы |

Подробнее: [BOT_COMMANDS.md](BOT_COMMANDS.md), [features/LAST_RUN_COMMAND.md](features/LAST_RUN_COMMAND.md)
